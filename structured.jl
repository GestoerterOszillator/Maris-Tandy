...
function Teil_Zwei(t, A, B)
    ...
    return A_itp, B_itp # Spline1D(t, A, k = 3), Spline1D(t, B, k = 3)
end
...
function Teil_Drei(A_itp, B_itp, Z_2, Z_4m; D = 0.93, w = 0.16)
    ...
    z_nodes, w_nodes = gausslegendre(50)

    function z_intA(p, q; c = 0.01)
        s = zero(Complex{Float64})
        @inbounds for i in eachindex(z_nodes)
            t = z_nodes[i]
            z = t + im*c*(1 - t^2)
            dzdt = 1 - 2im*c*t
            K = k2(p, q, z)
            pkv = pk(p, q, z)
            qkv = qk(p, q, z)
            integrand = sqrt(1 - z^2) * (p*q*z + 2*pkv*qkv/K) * alpha(K) / K * dzdt
            s += w_nodes[i] * integrand
        end
        return s
    end

    z, w_z = gausslegendre(40)
    z1  = 0.5 * z .- 0.5
    z2  = 0.5 * z .+ 0.5
    w_z = 0.5 * w_z;
    ...
    function Sigma_A(p2)
        ...
        return Z_2^2 * 16pi/(3*(2pi)^3 * p2) * quadgk(integrand, log(epsilon2), log(abs(p2)), log(Lambda2))[1]
    end
